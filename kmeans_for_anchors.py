...
if __name__ == '__main__':
    ...
    print('Load xmls.')
    data = load_data(path)
    # index = np.logical_and(data[:,0]<np.mean(data)*2,data[:,1]<np.mean(data)*2)
    # data = data[index]
    ...
    index_data = np.logical_and(data[:, 0] < np.mean(data) * 2, data[:, 1] < np.mean(data) * 2)
    index_cluster = np.logical_and(cluster[:, 0] < np.mean(data) * 2, cluster[:, 1] < np.mean(data) * 2)

    #-------------------------------------------------------------#
    #   绘图
    #-------------------------------------------------------------#
    colors = ['y','b','r','salmon','g','pink','cyan','gold','lime']
    fig, ax = plt.subplots()
    ax.spines['right'].set_color('none')
    ax.spines['top'].set_color('none')
    for j in range(anchors_num):
        plt.scatter(data[near == j][:,0], data[near == j][:,1],c=colors[j])
        plt.scatter(cluster[j][0], cluster[j][1], marker='x', c='black')
    plt.savefig("kmeans_for_anchors.png")
    plt.show()
    ...
    cluster = cluster[index_cluster]
    near = near[index_data]
    colors = ['y', 'b', 'r', 'salmon', 'g', 'pink', 'cyan', 'gold', 'lime']
    for j in range(anchors_num):
        try:
            plt.scatter(data[near == j][:, 0], data[near == j][:, 1], c=colors[j])
            ...
            pass
    plt.savefig("kmeans_for_anchors_expand.png")
    plt.show()
    print('Save kmeans_for_anchors.jpg in root dir.')
