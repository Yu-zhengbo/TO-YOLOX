...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        ratio = 1-(area_g/10000)
        ratio = torch.clip(ratio,0.2,2)
...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, strides=[4]):
        super().__init__()
    ...
    def forward(self, inputs, labels=None,adjacements=None):
        outputs             = []
        x_shifts            = []
        y_shifts            = []
        expanded_strides    = []
        gnn_inputs = inputs[0][:,-1,:,:]
        inputs = [inputs[0][:,:-1,:,:]]
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1)) + F.binary_cross_entropy_with_logits(gnn_inputs,adjacements)
