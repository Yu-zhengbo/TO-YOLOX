...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            image, box,adjacement  = self.get_random_data(self.annotation_lines[index], self.input_shape, random = self.train)
        image       = np.transpose(preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
        box         = np.array(box, dtype=np.float32)
        if len(box) != 0:
            box[:, 2:4] = box[:, 2:4] - box[:, 0:2]
            box[:, 0:2] = box[:, 0:2] + box[:, 2:4] / 2
        return image, box,adjacement
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
        ...
        if not random:
            ...
            if len(box)>0:
                ...
                adjacement = np.zeros((160, 160))
                box_ = box.copy()
                box_[:, 0] = (box_[:, 0] + box_[:, 2]) / 8
                box_[:, 1] = (box_[:, 1] + box_[:, 3]) / 8
                adjacement[box_[:, 1], box_[:, 0]] = 1
            else:
                adjacement = np.zeros((160, 160))

            return image_data, box ,adjacement
...
def yolo_dataset_collate(batch):
    ...
    bboxes = []
    adjacements = []
    for img, box, adj in batch:
        images.append(img)
        bboxes.append(box)
        adjacements.append(adj)
    images = np.array(images)
    adjacements = np.array(adjacements)
    return images, bboxes, adjacements


if __name__ == '__main__':
    with open('../2007_train.txt') as f:
        train_lines = f.readlines()
    train_dataset = YoloDataset(train_lines,  [640, 640], 2, 50 , mosaic=False,
                                train=True)
    for _,a in enumerate(train_dataset):
        print(1)
